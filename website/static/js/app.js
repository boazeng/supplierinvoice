...
const app = {
    ...
    renderInvoiceList(invoices) {
        ...
        if (invoices.length === 0) {
            container.innerHTML = '<tr><td colspan="9" class="empty-state">אין חשבוניות להצגה</td></tr>';
            return;
        }
    ...
    _renderJournalTable(box, d, branch) {
        ...
        box.querySelectorAll('.jl-acc').forEach(input => {
            ...
            input.addEventListener('blur',   () => {
                ...
                // המשתמש לא יכול להזין ערך חופשי — מקבלים רק בחירה מהרשימה.
                // אם הקליד טקסט שלא נבחר → מחזירים לערך השמור.
                const idx = parseInt(input.dataset.jli);
                const current = this.currentJournalLines[idx]?.account || '';
                if (input.value.trim() !== current) input.value = current;
            });
        ...
        const search = async (q) => {
            try {
                ...
                dd.innerHTML = items.map(doc => {
                    ...
                    const qty = Number(doc.TOTQUANT) || 0;
                    const subtotal = Number(doc.DISPRICE) || 0;
        ...
        dd.addEventListener('mousedown', (e) => {
            ...
            if (idx >= 0) {
                ...
                selected.push({
                    ...
                    booknum: doc.BOOKNUM || '',
                    curdate: doc.CURDATE || '',
                    quantity: doc.TOTQUANT || 0,
                    subtotal: doc.DISPRICE || 0,
