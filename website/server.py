...
async def one_time_fix_hanaya():
    ...
    companies = conn.execute(
        "SELECT id, name FROM ledger_companies ORDER BY id"
    ).fetchall()
    conn.close()

    # מוצא את שתי החברות: הקיימת (id נמוך) והחדשה שנוצרה אוטומטית (id גבוה)
    hanaya_companies = [dict(c) for c in companies
                        if "חניה" in c["name"] and "אורבנית" in c["name"]]
    if len(hanaya_companies) < 2:
        return {"ok": False, "error": "לא נמצאו שתי חברות חניה", "found": hanaya_companies}

    hanaya_companies.sort(key=lambda x: x["id"])
    correct_company = hanaya_companies[0]   # id נמוך = קיים מקודם
    wrong_company = hanaya_companies[-1]    # id גבוה = נוצר אוטומטית

    # מסמכים בחברה הלא-נכונה
    wrong_books = ledger_db.list_books(wrong_company["id"])
    moved_docs = []
    for book in wrong_books:
        docs = ledger_db.list_documents(book["id"])
        target_book_id = ledger_db.find_or_create_book(correct_company["id"], book["year"])
        for doc in docs:
            # חוצץ מתאים בחברה הנכונה
            invoice = store.get(doc["invoice_id"]) if doc.get("invoice_id") else None
            if invoice and invoice.extracted_data and invoice.extracted_data.supplier:
            ...
            div_name = ""
            if target_divider_id:
                divs = ledger_db.list_dividers(target_book_id)
                div_name = next((d["name"] for d in divs if d["id"] == target_divider_id), "")
            moved_docs.append({"doc_id": doc["id"], "title": doc.get("title"),
                                "divider": div_name or "ללא חוצץ"})

    # מחיקת החברה הלא-נכונה (ספרים + חוצצים + החברה עצמה)
    conn = ledger_db._conn()
    for book in wrong_books:
        conn.execute("DELETE FROM ledger_dividers WHERE book_id = ?", (book["id"],))
        conn.execute("DELETE FROM ledger_books WHERE id = ?", (book["id"],))
    conn.execute("DELETE FROM ledger_companies WHERE id = ?", (wrong_company["id"],))
    conn.commit()
    conn.close()

    return {
        "ok": True,
        "from_company": wrong_company["name"],
        "to_company": correct_company["name"],
        "moved_docs": moved_docs,
        "wrong_company_deleted": True,
    }
